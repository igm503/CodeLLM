...
def init_session_state():
    ...
    if "llm" not in st.session_state:
        st.session_state.llm = LLM(need_confirmation=False)
        ...
        with st.expander("Change Access Key"):
            ...
            if st.button("Submit"):
                if access_key != "":
                    ...
                    create_message("Access Key Changed Successfully", "System")
    else:
        access_key = st.text_input("Access Key", type="password")
        if st.button("Submit"):
            if access_key != "":
                st.session_state.llm.set_api_key(access_key)
                create_message("Access Key Set Successfully", "System")
...
def get_repo(
    ...
    if url is not None:
        try:
            ...
            create_message(
                f"Loading repo {st.session_state.repo_name} from URL {url}", "System"
            )
        except IndexError:
        ...
        create_message(
            f"Loading repo {st.session_state.repo_name} from URL {url}",
            "System",
        )
    else:
    ...
    try:
        ...
        create_message(
            f'Failed to fetch and embed "https://github.com/{author}/{name}.git"',
            "System}",
        )
        create_message(f"Error: {e}", "System")
        reset_repo_info()
        return
